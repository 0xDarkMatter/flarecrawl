...
|---------|------|---------|
| **v0.18.0** | 2026-04-18 | **Security hardening.** Closes path-traversal on `--resume JOB_ID`, blocks non-http(s) URLs (`file:`, `javascript:`, etc.) from entering the crawler, caps robots.txt and sitemap downloads so hostile servers can't OOM you. Also a round of refactoring: the crawl loop is now ~half its previous size and easier to extend. Ships with a PEP 561 `py.typed` marker so downstream projects get type hints. 1027 tests |
| **v0.17.0** | 2026-04-18 | **Industrial-scale crawling — new `flarecrawl authcrawl` command.** Drop-in successor to `authcrawl` built for crawls of millions of URLs across tens of thousands of domains. **Resume interrupted crawls** — kill the process, come back later, run `--resume JOB_ID`, pick up exactly where you left off. **Weekly refresh mode** — only re-fetch pages that changed since last run (`--refresh-days 7`); unchanged pages cost zero browser time via ETag/Last-Modified. **Fair scheduling** — round-robin across domains so one fast site can't starve 50,000 others. **Adaptive politeness** — `--adaptive-delay` automatically slows down on sites that respond slowly. **Auto-retry with backoff** and a dead-letter inspector (`flarecrawl frontier dead-letter JOB_ID`). **Circuit breaker** pauses domains after 10 consecutive failures. **Respects robots.txt** by default (protego). Under the hood: selectolax parser (10-30× faster than BeautifulSoup on hot paths), orjson, uvloop, tuned httpx pool — typical crawls are **2-5× faster** end-to-end. Full OpenTelemetry tracing (`--tracing console\|json\|otlp`) and Forma journal integration for production observability. URL canonicalisation with tracking-param stripping kills the `?utm_source=...` duplication problem. 967 tests |
