...
|---------|------|---------|
| **v0.30.0** | 2026-06-01 | **Local tech detection.** New `flarecrawl tech-detect URL` subcommand (with `--min-confidence`, `--only-categories`, `--exclude-categories`, parallel `-i FILE -w N`, `--ndjson` streaming, `--stdin` piped HTML) + `--tech-detect` flag on `scrape`, `crawl`, `fetch` + `Client.detect_tech()` Python API. Wappalyzer fingerprint matching over each command's collected data: HTML always, plus HTTP response headers and cookies (full coverage on the dedicated subcommand, `fetch`, and CDP-mode `scrape` via `Network.responseReceived` + `CDP.getCookies`; REST `scrape` side-fetches via a streaming GET with 32 KB cap; `crawl` is HTML-only per record), plus a `Runtime.evaluate`-injected JS-globals probe on CDP `scrape` (~5,500 property paths). Vendored DB: ~7,500 upstream techs (`enthec/webappanalyzer`, GPL-3.0 — see `wappalyzer_data/LICENSE.wappalyzer_data`) + 60-entry overlay (Craft CMS, Tailwind CSS, hospitality/tourism booking engines: SevenRooms, ResDiary, OpenTable, Mr Yum, me&u, Rezdy, FareHarbor, SiteMinder). Attaches `technologies: [...]` to JSON output; compact `[tech]` stderr summary on non-JSON. Discoverable via `flarecrawl guide tech-detect`. Thread-safe singleton, lazy-loaded. Zero CF browser time. 35 new tests including a local HTTP fixture server. |
