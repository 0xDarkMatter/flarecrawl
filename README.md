...
|---------|------|---------|
| **v0.18.0** | 2026-04-18 | **Hardening + architecture polish.** Security: `job_id` regex validator closes path-traversal in `Frontier.open`/`--resume`/`frontier dead-letter`, http(s) scheme allow-list in `canon.canonicalize` rejects `file:`/`javascript:`/`data:`/`gopher:`, response size caps on robots.txt (2 MB) and sitemap (10 MB). DRY: shared `src/flarecrawl/_http.py` (`polite_get`/`ensure_client`/`origin`) consumed by `robots.py` and `sitemap.py`, `AuthenticatedCrawler._error_result` collapses 5 duplicate failure branches, narrowed `except` tuples in robots + authcrawl with debug logging. Architecture: `AuthenticatedCrawler.crawl()` split into named phases (`_seed_frontier`/`_prefilter_batch`/`_run_batch`/`_ingest_results`/`_emit_terminal`/`_finalise`) — cyclomatic complexity 34 → 12, length 148 → 73 lines. Telemetry `_try_import` helper factors OTel optional-import boilerplate. Frontier `_trip_sick_if_threshold` helper. `mark_unchanged` collapsed into `mark_done`. PEP 561 `py.typed` marker shipped in wheel. 1027 tests |
| **v0.17.0** | 2026-04-18 | **Performance campaign landing.** uvloop conditional bootstrap (non-Windows), selectolax across `extract`/`openapi`/`authcrawl`/`cli` HTML parsing, orjson via `json_compat`, `@dataclass(slots=True)` on 13 hot classes, httpx connection-pool tuning (100 keepalive / 50 max), per-domain `DomainRateLimiter`, protego robots.txt honouring, polite default `FlarecrawlBot` User-Agent, **frontier v2** (URL canonicalisation with tracking-param deny-list, blake2b-16 fingerprint dedup, per-host round-robin scheduler, retry budget with exponential backoff, snooze vs sick states, adaptive EWMA delay, crash-resume via `rollback_in_flight`, dead-letter sink, circuit breaker), OpenTelemetry tracing with httpx instrumentation (`none`/`console`/`json`/`otlp` exporters), Forma journal integration (`forma log emit` → NDJSON fallback), new `flarecrawl authcrawl` subcommand exposing `--resume` / `--max-attempts` / `--adaptive-delay` / `--refresh-days` / `--tracing`, plus cleanup of the orphan `_fetch_page` helper. 967 tests |
| **v0.16.0** | 2026-04-17 | **Design system extraction.** `flarecrawl design extract` generates DESIGN.md from any website — colors, typography, spacing, shadows, radii, layout, CSS variables, media queries, z-index. 9-category Design Coherence scoring (A-F). `design coherence` for standalone scoring. `design diff` for side-by-side comparison. HTML preview with visual swatches. Works behind auth via --session. Uses CDP for live computed style extraction |
