...
|---------|------|---------|
| **v0.28.0** | 2026-05-16 | **Hard-target field-report closure.** New `flarecrawl p6` mint→replay primitive (local-Chromium cookie-shell mint → curl_cffi TLS replay, proactive re-mint, cumulative egress cool-down, terminal CF-1020 fast-fail). Machine-readable `meta.blocked` (Akamai/Cloudflare/Imperva/DataDome/PerimeterX/CloudFront). `flarecrawl session inspect` offline jar freshness. Windows cp1252 output crash fixed. `--json` no longer overrides `--output`/backend. HAR flushed on selector timeout. Recipe capture pre-armed before nav; `schema_version: 1`. 820 tests |
