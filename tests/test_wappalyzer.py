...
# ---------------------------------------------------------------------------
# tech-detect --cdp (CF Browser Run JS-globals probe path)
#
# The `--cdp` flag routes the tech-detect command through the same CF Browser
# Run + Runtime.evaluate JS-globals probe machinery the v0.30.0 scrape path
# uses. End-to-end coverage lives in tests/live/ (needs CF auth + network);
# the helper is exercised offline via the mocked CDP unit test below.
# ---------------------------------------------------------------------------


def test_fetch_for_tech_detect_cdp_calls_probe_and_returns_tuple(monkeypatch):
    """Unit: `_fetch_for_tech_detect_cdp` runs the probe, parses globals,
    returns the (html, headers, cookies, js_globals) tuple shape."""
    from flarecrawl import cli as _cli
    from flarecrawl.cdp import MainDocumentHeaders

    fake_html = "<html><head><title>x</title></head><body>y</body></html>"
    probe_payload = '{"testSignal": "match", "window.foo": null}'
    ...
    class _FakePage:
        def __init__(self) -> None:
            ...
            self.cookies_set: list[dict] | None = None
            self.headers_set: dict | None = None
        ...
        def send(self, method: str, params: dict | None = None) -> dict:
            if method == "Network.setExtraHTTPHeaders":
                self.headers_set = dict((params or {}).get("headers") or {})
            return {}

        def set_cookies(self, cookies) -> None:
            self.cookies_set = list(cookies)

        def navigate(self, url, **_) -> dict:
            return {}
        ...
        def evaluate(self, expr: str, await_promise: bool = True):
            ...
            # First call is the probe injection; second reads #wap-probe.
            if "wap-probe" in expr:
    ...
    class _FakeClient:
        ...
        def subscribe(self, event: str, handler) -> None:
            ...
            # Simulate a Network.responseReceived event for the main doc so
            # the MainDocumentHeaders collector populates.
    ...
    assert "window.foo" in js_globals
    assert fake_page.network_enabled
    assert fake_page.headers_set == {"X-Custom": "yes"}
    assert fake_page.cookies_set is not None
    assert fake_page.closed
    assert fake_client.closed
    # Probe was injected then read back.
    assert any("wap-probe" in c for c in fake_page.evaluate_calls)
